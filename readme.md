...
- 더 많은 feature를 학습에 사용하기
  - 후술할 모든 개선점의 시발점임. 학습에 사용된 특성 선택을 **직관에 따라** 극히 일부로 제한했는데, `SelectKBest` 모듈을 사용하여 모든 특성이 각각 Label Data와 어느 정도의 연관성을 갖는지 수치화하여, 상위 특성들을 남기는 방법들을 사용할 수도 있음.
...
- 개봉 전에 알 수 있는 특성만으로 학습하기
  ...
  - 따라서 '개봉 전'을 상정하고, 개봉 전에 알 수 있는 특성만으로 모델 학습을 하는 것도 좋은 개선방안이 될 수 있음.
 
- 군집화된 데이터의 특성을 파악하기
  - PCA를 적용하여 데이터의 분포를 관찰했을 때, 클러스터링이 무의미한 수준으로 보였음.
  	- 이는 데이터가 어떠한 경향을 일관되게 나타내고 있기 때문인데, 해당 프로젝트에서 사용하지 않은 특성을 추가로 투입한다면 유의미한 결과를 도출할 수도 있음.
   	- 예를 들어, `genre` 특성을 해당 분석에서 사용하지 않은 `director` 특성과 관련지었다면, 같은 장르여도 감독이 다른 경우 더 높은 수익을 기록하는 경향을 보일 수도 있음.
        
  - 클러스터의 갯수 조정을 수치만으로 조정하면 인간의 직관과는 다른 결과가 나옴.
    - k-means의 경우, 클러스터 간의 명확한 구분 기준을 찾기 어려울 정도로 중첩되어 보이는 구간이 많음.
    - DBSCAN의 경우, 데이터의 밀집도를 기준으로 clustering을 시행하기 때문에, ***(outlier를 제외한 데이터들을 하나의 cluster로 묶는 것이 옳아 보이는데도 불구하고 << 이 부분은 지워주시고 발표할 때 읽어주시면 됩니다)*** 억지로 클러스터 갯수를 늘려서 효과가 미미함.
