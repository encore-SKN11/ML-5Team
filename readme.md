...
- 군집화된 데이터의 특성을 파악하기
  - PCA를 적용한 데이터의 분포를 관찰했을 때, 클러스터링이 무의미한 수준으로 보였음.
  	- 이는 모델 학습에 사용한 feature들이 어떠한 경향을 일관되게 나타내고 있기 때문인데, 해당 프로젝트에서 사용하지 않은 특성을 추가로 투입한다면 유의미한 결과를 도출할 수도 있음.
   	- 예를 들어, `genre` 특성을 해당 분석에서 사용하지 않은 `director` 특성과 관련지었다면, 같은 장르여도 감독이 다른 경우 더 높은 수익을 기록하는 경향을 보여 유의미한 클러스터링 결과를 보였을 수도 있음.
