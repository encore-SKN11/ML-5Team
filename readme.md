...
- 군집화된 데이터의 특성을 파악
  ...
  - 클러스터의 갯수 조정을 수치만으로 조정하면 인간의 직관과는 다른 결과가 나옴.
    ...
    - DBSCAN의 경우, 데이터의 밀집도를 기준으로 clustering을 시행하기 때문에, (outlier를 제외한 데이터들을 하나의 cluster로 묶는 것이 옳아 보이는데도 불구하고 << 이 부분은 지워주시고 발표할 때 읽어주시면 됩니다) 억지로 클러스터 갯수를 늘렸기 때문에 효과가 미미함.
